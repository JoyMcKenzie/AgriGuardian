...
function inviteMember() {
  ...
  // Least privilege: minting an elevated account is Owner-only. A Manager can
  // invite roles at or below their level (Technician, Farm Hand, Viewer) but
  // not another Manager or the Owner.
  if ((role === 'Manager' || role === 'Owner') && currentUser.role !== 'Owner') {
    alert('Only the Owner can invite a Manager. You can invite a Technician, Farm Hand, or Viewer.');
    return;
  }
  ...
  setTimeout(function() {
    teamMembers.push({ name: name, phone: phone, role: role, status: 'Invited', archived: false, perms: roleTemplate(role) });
...
function saveMemberEdits(phone) {
  ...
  if (!m) return;
  // Name and phone are identity fields — and the phone is the login/invite
  // vector — so editing them is Owner-only, enforced at the action.
  if (currentUser.role !== 'Owner') { alert('Only the Owner can change a member\u2019s name or phone number.'); return; }
  ...
  if (newPhone !== oldPhone && !window.confirm('Change ' + (oldName || oldPhone) + '\u2019s phone to ' + newPhone + '?\n\nThe phone number is how they sign in, so this changes their login.')) return;
  m.name = newName;
  m.phone = newPhone;
  logAction('Team member updated', oldName + ' \u2192 name: ' + newName + ', phone: ' + newPhone);
  _refreshMemberPanel(newPhone);
