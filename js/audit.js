...
function renderAuditLog() {
  ...
  return auditLog.map(e =>
    '<div style="padding:8px 0;border-bottom:1px solid #E4EEE4;font-size:12px">' +
      '<div style="display:flex;justify-content:space-between;margin-bottom:2px">' +
        '<span style="font-weight:600;color:#22372A">' + e.action + '</span>' +
        '<span style="color:#7A8F80">' + e.ts + '</span>' +
      '</div>' +
      '<div style="color:#5F7266">' + e.actor + ' (' + e.role + ')' + (e.detail ? ' — ' + e.detail : '') + '</div>' +
    '</div>'
