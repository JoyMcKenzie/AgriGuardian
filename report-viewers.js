...
function _openReportViewer(titleText, bodyHtml) {
  ...
  if (!overlay) {
    ...
    overlay.innerHTML =
      '<div style="background:#fff;max-width:520px;width:100%;max-height:90vh;border-radius:12px;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 10px 40px rgba(0,0,0,0.25)">' +
...
function viewHygieneReport() {
  ...
  var scColor = h.overall >= 80 ? '#2E7A4E' : h.overall >= 50 ? '#C9A400' : '#C0392B';
  var bar = function(label, val){
    return '<div style="margin:6px 0"><div style="display:flex;justify-content:space-between;font-size:12px;color:#555"><span>' + label + '</span><span>' + val + '%</span></div>' +
  ...
  var html = '<div style="color:#888;font-size:12px;margin-bottom:10px">' + farm + ' &middot; ' + t('pdfGenerated') + ' ' + dateStr + '</div>' +
    '<div style="border:1px solid #e0e0e0;border-radius:10px;padding:14px;margin-bottom:14px;display:flex;align-items:center;gap:14px">' +
  ...
  if (openIssues.length === 0) {
    html += '<div style="color:#666;font-size:13px;margin-bottom:14px">' + t('pdfNoActions') + '</div>';
  } else {
    html += '<ul style="padding-left:18px;margin-bottom:14px">' + openIssues.map(function(d){
      ...
      return '<li style="margin-bottom:6px">' + (d.label || d.type) + ' (' + d.brand + ') &mdash; ' + getRiskAction(r, d.pw, d.brand) +
        (d.assignedTo ? ' <span style="color:#888">[' + t('assignedToLabel') + ': ' + d.assignedTo + ']</span>' : '') + '</li>';
    }).join('') + '</ul>';
  }
  html += '<div style="font-weight:600;color:#1F4D2E;margin-bottom:6px">' + t('pdfDeviceInventory') + ' (' + active.length + ')</div>' +
    '<table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:10px">' +
    '<tr style="background:#f2f6f3;color:#1F4D2E"><th style="text-align:left;padding:6px">' + t('pdfDevice') + '</th><th style="text-align:left;padding:6px">' + t('brandLabel') + '</th><th style="text-align:left;padding:6px">' + t('pdfStatus') + '</th></tr>' +
    active.map(function(d){
      var r = getRisk(d.brand, d.pw, d.healthStatus);
      var statusTxt = d.resolved ? t('pdfResolved') : (r === 'red' ? t('pdfHigh') : r === 'yellow' ? t('pdfMedium') : t('pdfLow'));
      var color = d.resolved ? '#2E7A4E' : (r === 'red' ? '#C0392B' : r === 'yellow' ? '#C9A400' : '#2E7A4E');
    ...
    '<div style="font-size:11px;color:#999;margin-top:12px;border-top:1px solid #eee;padding-top:8px">' + t('pdfDisclaimer') + '</div>';
...
function viewActivityReport() {
  ...
  var farm = (currentUser && currentUser.farm) ? currentUser.farm : t('pdfYourFarm');
  var dateStr = new Date().toLocaleDateString(currentLang === 'es' ? 'es-ES' : 'en-US', { year:'numeric', month:'long', day:'numeric' });
  var html = '<div style="color:#888;font-size:12px;margin-bottom:10px">' + farm + ' &middot; ' + t('pdfGenerated') + ' ' + dateStr + ' &middot; ' +
    auditLog.length + ' ' + (auditLog.length === 1 ? t('auditAction') : t('auditActions')) + '</div>';
  if (auditLog.length === 0) {
    html += '<div style="color:#999;font-size:13px">' + t('pdfNoActivity') + '</div>';
  } else {
    html += '<table style="width:100%;border-collapse:collapse;font-size:12px">' +
      '<tr style="background:#f2f6f3;color:#1F4D2E"><th style="text-align:left;padding:6px">' + t('pdfWhen') + '</th><th style="text-align:left;padding:6px">' + t('pdfWho') + '</th><th style="text-align:left;padding:6px">' + t('pdfWhat') + '</th></tr>' +
      auditLog.map(function(e){
        return '<tr style="border-top:1px solid #eee"><td style="padding:6px;color:#555;white-space:nowrap">' + (e.ts || '') + '</td>' +
          '<td style="padding:6px">' + e.actor + ' <span style="color:#888">(' + e.role + ')</span></td>' +
          '<td style="padding:6px">' + e.action + (e.detail ? ' &mdash; <span style="color:#666">' + e.detail + '</span>' : '') + '</td></tr>';
      }).join('') + '</table>';
  }
  html += '<div style="font-size:11px;color:#999;margin-top:12px;border-top:1px solid #eee;padding-top:8px">' + t('pdfActivityDisclaimer') + '</div>';
